...
module.exports = React.createClass({
  ...
  _getElement: function() {
		...
		if (this.props.label.length > 0) {
			children.push(<label className="mdl-textfield__label" htmlFor={this.props.id}>{this.props.label}</label>);
		}

		if (this.props.error.length > 0) {
			children.push(<label className="mdl-textfield__error">{this.props.error}</label>);
		}

		if (this.props.expandable > 0) {
			children = [
				<label className="mdl-button mdl-js-button mdl-button--icon" htmlFor={this.props.id}><i className="material-icons">{this.props.icon}</i></label>,
				<div className="mdl-textfield__expandable-holder">{children}</div>
			];
		}
